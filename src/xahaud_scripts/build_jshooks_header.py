...
def get_qjsc_hash(qjsc_path):
    ...
    try:
        with open(qjsc_path, 'rb') as f:
            return hashlib.sha256(f.read()).hexdigest()[:16]  # Use first 16 chars of hash for brevity
...
@click.command()
@click.option('--canonical', is_flag=True, help='Use canonical mode: output .sh in header and disable cache')
...
std::map<std::string, std::vector<uint8_t>> jswasm = {{''')
    ...
    for hook_content in processed_matches:
        ...
        if not canonical and cache_dir:
            ...
            content_hash = hashlib.sha256(f"{qjsc_hash}:{hook_content}".encode('utf-8')).hexdigest()
    ...
    try:
        ...
        logging.warning("Warning: clang-format failed, output might not be properly formatted")
