...
"""
    ...
    def write_python_manifest(
        ...
        """Write a Python dict manifest alongside the .h file.

        Same keys as the C++ map, values are hex strings of the bytecode.
        """
        py_file = self.output_file.with_suffix(".py")
        lines = [
            "# Generated by x-build-test-hooks\n",
            f"{self.symbol_name} = {{\n",
        ]
        for counter in sorted(compiled_blocks.keys()):
            block, bytecode = compiled_blocks[counter]
            key = repr(block.map_key)
            lines.append(f'    {key}: "{bytecode.hex()}",\n')
        lines.append("}\n")
        content = "".join(lines)

        if py_file.exists() and py_file.read_text() == content:
            logger.info("Python manifest unchanged, skipping write")
            return

        logger.info(f"Writing {py_file}")
